...
public class MetaController {
    ...
    public String excluir(@PathVariable Long id, @AuthenticationPrincipal Usuario usuario, Model model) {
        try{
            ...
            model.addAttribute("erro", "Você não tem permissão para deletar esta meta");
            return "redirect:/metas";
        }
    }
    ...
    public String editar(@PathVariable Long id, Model model, @AuthenticationPrincipal Usuario usuario) {
        try {
            ...
            model.addAttribute("erro", "Você não tem permissão para editar essa meta");
            return "redirect:/metas";
    ...
    @PutMapping
    public String atualizar(@Valid @ModelAttribute("meta") NovaMetaDTO request, BindingResult result, Model model, @AuthenticationPrincipal Usuario usuario) {
        if(result.hasErrors()) {
            return "meta/criar";
        }
